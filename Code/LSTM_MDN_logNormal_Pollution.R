...
lags <- 10 #lags = 0 corresponds to just using the current TS val to forecast
lookAhead <- 10
...
callback <- list(keras::callback_reduce_lr_on_plateau(monitor = "val_loss", factor = lrReduction, patience = patiencePlateau), keras::callback_early_stopping(monitor = "val_loss", patience = patienceEarlyStopping),
                 keras::callback_model_checkpoint(filepath = paste0(saveDir, "/", "lorenz96_mdn_weights.hdf5"), save_best_only = TRUE, save_weights_only = TRUE))
...
load_model_weights_hdf5(model_mdn, paste0(saveDir, "/", "lorenz96_mdn_weights.hdf5"))

outputs_test_mdn <- predict(model_mdn, X.test.scaled)

Y.test.pred <- outputs_test_mdn
for(i in 1:nrow(Y.test.pred))
{
	Y.test.pred[i, ] <- Y.test.pred[i, ]*(rnnData$colMaxs.ts - rnnData$colMins.ts)
}
