...
for(target_hour in 1:24)
{
  ...
  #for(d_lag in 1)
  {
    ...
    callback_specs=list(callback_early_stopping(monitor = "val_loss", min_delta = 0, patience = 10,
                                                verbose = 0, mode = "auto"),
                        callback_model_checkpoint(filepath='best_model_wind.hdf5',save_freq='epoch' ,save_best_only = TRUE)
    )
    ...
    model_best = load_model_hdf5('best_model_wind.hdf5',compile=FALSE)
...
save(file='ANN_Wind_dropout.Rdata',long.term.pred,long.term.lower,long.term.upper,y.test.mat)
